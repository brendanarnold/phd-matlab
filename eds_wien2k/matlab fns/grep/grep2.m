...
function	p=get_files(p,buff)
	...
	if	tf
        ...
        filesize = s.bytes;
        if filesize>buff
            notatend=true; offset=0;
            while notatend
                p=get_file(p,offset,buff);
                offset=offset+buff-numel(p.pattern{1});
                notatend=offset<=filesize;
            end
        else
            p=get_file(p);
        end
		...
		return;
%--------------------------------------------------------------------------------
function	p=get_file(p,varargin)
...
% incrementally include/exclude files/full paths
	...
	if	~ixi		||...
		...
		return;
%--------------------------------------------------------------------------------
function	p=get_match(p,varargin)
if nargin==3
    offset=varargin{1};
    buff=varargin{2};
else
...
% find string pattern <str>
	if	p.opt.R.flg
        ...
        p.byteoffs=[p.byteoffs; offset+ix'];
		p.par.nmatch=0;
	if	~isempty(ix)
