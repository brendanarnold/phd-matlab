function hall_waterfall_plot(y_col)
...
%     input_extension = '.*_(.*)-.*_spliced\.dat\.avg$';
    ...
    for index = 1:num_plots
        ...
        %smoothed_y = smooth(file_struct.data(:,y_col), 19, 'moving'); % % point avg smooth
        %diff_y = diff(smoothed_y)/0.2;
        %diff_y = smooth(diff_y, 19, 'moving');
        diff_y = file_struct.data(:,y_col);
        %x_vals = file_struct.data(1:end-1,x_col);
        x_vals = file_struct.data(:,x_col);
        lines(index) = line(x_vals, diff_y, 'Color', line_color);
