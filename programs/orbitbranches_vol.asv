...
%nov06 modifying so can specify volume for orbit centres and volume for whole orbits
%FS.centresvol, FS.searchvol
...
if ~exist('kparas') || isempty(kparas)
    ...
    %kpara range is found by calc intersection of normal with all planes that bound centresvol
    %note does not work if search volume is not centred on Gamma point
    if isempty(bandsdata.centresvol)
        bvecs=bandsdata.BZfacenormals*bandsdata.rec_latt_vecs;
    else
        bvecs=bandsdata.centresvol;
    end
    ...
    kparas=kparamin-extra_kpara:dkpara:kparamax+extra_kpara;
else
...
for bandnum=1:length(bandnums)
    ...
    if ~isempty(allorbits{bandnum})
        ...
        branchnumbers=grouporbits_test(allorbits{bandnum},kparas,6,2,fdiff_threshold,3,kdiff_threshold,1);
