...
for bnum=1:numel(bands.cartE)
    ...
    if numel(find(ismem))>0
        totDOStable(loc(loc~=0),col)=totDOStable(loc(loc~=0),col)+DOS(ismem)';
    end        
    lastind=size(totDOStable,1);
    totDOStable(lastind+1:lastind+numel(find(~ismem)),:)=[E(~ismem)' zeros(numel(find(~ismem)),2)];
    totDOStable(lastind+1:lastind+numel(find(~ismem)),col)=DOS(~ismem)';    
end
