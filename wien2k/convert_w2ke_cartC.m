function convert_w2ke_cartC(pathcasename,bandnumlist,ForceFermiLevel,interpmethod,outfilename,varargin)
%function convert_w2ke_inds(pathcasename,bandnumlist,ForceFermiLevel,interpmethod,outfilename);
%
% Behaves as convert_w2ke but also includes cartC, several cartesian grids of band
% characters. To work you need the case.band_x files generated by Tony's
% Delphi program in the same directory. This does not work with spin orbit
% calculations at present. Format of cartC is of a cell array containing
% one cell for each character specified in the bands_X files, inside of
% this is another cell array for each band similar to cartE.
%
...
b.units = linestr{3}(24:26);
% i.e. latt_params = [kx ky kz] in angstrom
b.latt_params = [str2num(linestr{4}(1:10)) str2num(linestr{4}(11:20)) str2num(linestr{4}(21:30))];
if ~strcmp(b.units, 'ang')
    b.latt_params = Bohr_radius * b.latt_params;
end
% i.e. latt_angles = [alpha beta gamma] in deg
...
% Normalise the reciprocal lattice vectors to +-1
denom = max(abs(b.rlvs));
...
fclose(infile);
% Spanning vectors reach across the unit cell?
...
%read energies *only* from case.energy files
for dirnnum=1:length(dirnstr)    
    if nobandranges
...
if strcmp(lower(interpmethod),'fourier')
    ...
    % Start cartC hack to interpolate the characters onto the meshes
    % FIXME: Spin polarised calculations are not dealt with i.e.
    % case.qtlup, case.qtldn
    dirnnum = 1;
    char_data = [];
    for band_ind = 1:length(bandnumlist{dirnnum})
        char_data_cols = load([pathcasename '.band_' num2str(band_ind)]);
        % Make room to fit for all bands (i) all kpts (j) for each of their
        % band characters (k)
        if isempty(char_data)
            char_data = zeros(length(bandnumlist{dirnnum}), size(char_data_cols, 1), size(char_data_cols, 2) - 4);
        end
        % Discard the kx ky kz and energies columns
        char_data(band_ind,:,:) = char_data_cols(:,5:end);
    end
    b.cartC = {};
    for char_ind = 1:size(char_data, 3)
        disp(['Calculating character: ' num2str(char_ind)]);
        [dummy dummy dummy b.cartC{char_ind}] = orthomeshe_griddata(squeeze(char_data(:,:,char_ind)), ...
            b.uc_klist, b.W2kdims, b.LCMdivs, b.latt_type, b.n_rlvs, b.rlv_mags, b.cart_submesh_shift, b.mcell);
        clear dummy;
    end
    % End hack
end
